...
export const GamifiedView: React.FC<ViewProps> = ({ cornCount, handleBuy, loading, cooldown }) => {
  // Calculamos el porcentaje para la barra de progreso (60s = 100%)
  const progress = Math.min(100, ((60 - cooldown) / 60) * 100);

  return (
    <div className="h-full p-6 flex flex-col items-center bg-gradient-to-b from-blue-50 to-green-50 rounded-xl overflow-hidden relative">
      {/* Cielo y Sol */}
      ...
      <div className="z-10 text-center mt-8 mb-12">
        <h2 className="text-3xl font-bold text-green-900 mb-2">Granja de Bob</h2>
        <Badge variant="success">Nivel {Math.floor(cornCount / 5) + 1}</Badge>
      </div>

      {/* Visualización del Maíz */}
      <div className="flex-1 flex flex-wrap justify-center content-start gap-2 max-w-md overflow-y-auto p-4" style={{maxHeight: '300px'}}>
        {Array.from({ length: cornCount }).map((_, i) => (
          <div key={i} className="animate-in zoom-in duration-300">
            <Wheat className="w-8 h-8 text-yellow-500 drop-shadow-sm" fill="currentColor" />
          </div>
        ))}
        {cornCount === 0 && <span className="text-zinc-400 italic">Tu granero está vacío...</span>}
      </div>
      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-white/50 mt-auto z-10">
        <div className="flex justify-between text-sm font-medium text-zinc-600 mb-2">
          <span>Energía de Cosecha</span>
          <span>{cooldown > 0 ? 'Recargando...' : '¡Listo!'}</span>
        </div>
